    ...
    IF EXISTS (SELECT 1 FROM items WHERE items.visitor_id = v_id) THEN
        RAISE EXCEPTION 'visitor_already_has_item';
    END IF;

    -- try to assign item if unassigned
    UPDATE items SET visitor_id = v_id WHERE id = _item_id AND items.visitor_id IS NULL;
    IF NOT FOUND THEN
    ...
    SELECT items.visitor_id INTO current_owner FROM items WHERE id = _item_id;
